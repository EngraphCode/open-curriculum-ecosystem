...
export const toolMcpFlatInputSchema = z.strictObject({ keyStage: z.enum(["ks1", "ks2", "ks3", "ks4"] as const).describe("Key stage slug to filter by, e.g. 'ks2' - note that casing is important here, and should be lowercase").meta({ examples: ["ks1"] }), subject: z.enum(["art", "citizenship", "computing", "cooking-nutrition", "design-technology", "english", "french", "geography", "german", "history", "maths", "music", "physical-education", "religious-education", "rshe-pshe", "science", "spanish"] as const).describe("Subject slug to filter by, e.g. 'english' - note that casing is important here, and should be lowercase").meta({ examples: ["english"] }), unit: z.string().describe("Optional unit slug to additionally filter by").meta({ examples: ["word-class"] }).optional(), offset: z.preprocess((val) => typeof val === 'string' && /^-?\d+(\.\d+)?$/.test(val) && Number.isFinite(Number(val)) ? Number(val) : val, z.number().describe("Offset applied to lessons within each unit (not to the unit list).").meta({ examples: [11] })).optional(), limit: z.preprocess((val) => typeof val === 'string' && /^-?\d+(\.\d+)?$/.test(val) && Number.isFinite(Number(val)) ? Number(val) : val, z.number().lte(300).describe("Limit the number of lessons returned per unit. Units with zero lessons after limiting are omitted.").meta({ examples: [10] })).optional() });
