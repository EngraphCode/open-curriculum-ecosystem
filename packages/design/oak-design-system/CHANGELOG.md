...
- **`--type-heading-1..3` are now assembled by the kit from brand-writable parts** — `--type-heading-N-min` / `-max` (unitless numbers of rem) and `--type-heading-N-leading` (unitless ratio). The kit derives the whole fluid curve from the bounds: min renders at a 320px viewport, max from 960px up, both bounds rem-true under text resize. Weight and family stay on `--weight-display` / `--font-display`. Brands set two numbers per slot to reshape the ramp; declaring the whole slot shorthand remains a working opt-out (it wins by cascade) — never declare both: the shorthand wins and the parts become silent dead code, and no gate catches that today (authoring rule, unenforced; admission checking arrives with the identity-pack manifest, owner ruling 2026-08-17). `brand.css` §1b carries the contract.
