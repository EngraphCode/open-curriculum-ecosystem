...
Module/package boundaries follow cohesion, explicit dependencies, platform needs, release responsibility and useful consumption. Importing one atom must not load unrelated engines. OCE additionally binds the container: the workspaces that hold atoms declare a class whose directories and files are budgeted by a repository validator and whose lint, compiler and assurance tiers are stricter than the estate's — an adopting-repository binding recorded in ADR-166 and ADR-230, of the same kind as the zero-runtime-dependency default in §2. The atom's own boundary remains the responsibility judgement R01 states; the binding bounds its home, not its definition. Capability need can be owner-established before several consumers exist; evidence about consumer use helps test the boundary and sequence implementation, not invalidate legitimate innovation.
