...
**70 items.** Of those, 0 are traced to a surface an agent can reach today, 0 to a surface that is retained but switched off, 0 to both a reachable and a switched-off surface, and 21 no longer exist in the codebase. 49 live in code that ships, but this pass has not traced which registered surface carries them — each says so.
