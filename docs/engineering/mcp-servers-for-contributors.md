...
This fork's tracked `.claude/settings.json` activates no plugin (owner ruling,
2026-09-08: the project-level plugin activation was simplified away). Each
contributor installs the sanctioned plugins they use once, with Claude Code's
plugin command (`/plugin install <plugin>@claude-plugins-official`); the rows
below name the plugin where one applies.

| Server                              | Purpose                                                                    | How to enable                                                                                                                                                                                                                                                                               |
| ----------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **SonarQube**                       | Code quality and security scanning aligned with Oak's SonarQube project    | Install the plugin (`sonarqube@claude-plugins-official`), then `/sonarqube:sonar-integrate` (Claude Code) installs the CLI and integration; ask the team for the project access token                                                                                                       |
...
| **Sentry (Oak Open Curriculum)**    | Sentry issues, events, releases, and Seer for the Open Curriculum projects | Install the plugin (`sentry@claude-plugins-official`), authenticate it against the Oak Sentry organisation; ask the team for org access. MCP server development uses `mcp-server-dev@claude-plugins-official` the same way                                                                  |
...
| **Linear**                          | Issue tracking: issues, projects, cycles, comments in the Oak workspace    | Upstream line only: the upstream repository's tracked `.claude/settings.json` enables the Linear MCP plugin (Claude Code offers to install it on first launch); authenticate via `/mcp`. This fork does not use Linear (owner ruling, 2026-09-06) and its settings file activates no plugin |
