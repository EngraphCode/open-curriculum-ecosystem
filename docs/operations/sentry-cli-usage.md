...
| E2E checks (today `*.e2e.test.ts` files, a name the recovery plan retires)     | **No**                  | Per testing-strategy.md, an E2E check drives a separately running system over its own protocol channel, never a vendor network.                                                                                                                                                                                                                                                                                                  |
...
| Network-reaching smoke checks (explicitly invoked, never on the PR-check path) | **Yes, on-demand only** | Smoke checks are validation surfaces, not tests (testing-strategy.md); one that reaches a vendor network runs on demand only (ADR-161). A smoke check that exercises `sentry-cli send-event` or similar is legitimate here but must be explicitly triggered, never attached to the PR gate.                                                                                                                                      |
