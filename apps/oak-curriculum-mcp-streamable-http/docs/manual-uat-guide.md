...
| 10.4 | `ui://widget/oak-curriculum-app.html` | `resources/read`                            | `text/html;profile=mcp-app` widget document (the MCP App surface).                                                 |
| 10.5 | Same address, unauthenticated         | `resources/read` with no token (Appendix B) | The widget document; the address is the one `get-curriculum-model` advertises in `_meta.ui.resourceUri` (ADR-141). |
