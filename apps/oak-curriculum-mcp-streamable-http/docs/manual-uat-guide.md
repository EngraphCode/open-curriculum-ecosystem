...
| 10.6 | A retired address, unauthenticated       | `resources/read` of `ui://widget/oak-curriculum-app-899803c6.html` with no token (Appendix B) | HTTP `200` carrying JSON-RPC error `-32602` (resource not found), not `401`: a client holding the address is told to list tools again rather than to sign in (ADR-141).                                                                                                                 |
