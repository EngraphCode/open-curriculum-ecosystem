...
| `ELASTICSEARCH_URL`     | ✅         | ✅      | Search cluster endpoint; the server refuses to boot without it, and the deploy-config gate fails the build on its absence (MCP-475).                                         |
| `ELASTICSEARCH_API_KEY` | ✅         | ✅      | Search cluster API key; required alongside `ELASTICSEARCH_URL`, and build-blocking through the same gate.                                                                    |
