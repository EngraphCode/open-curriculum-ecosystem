...
| 2026-09-10 | The consolidation seat's #109 landing attempts (03:1xZ–04:0xZ) | The Codex connector binds a pull request at a creation or push event it observes with an unseen diff: it reviewed #110 at creation (opened non-draft, four minutes) and on its slot push, and #105/#108 on pushes after their draft creation; it never reviewed #109 (not at open, not on two pushes, not on an "@codex review" comment) nor its non-draft successor #113 (creation and a push) — the two source PRs of one branch. Copilot, by contrast, is bindable on any head by the bot's explicit request (`POST pulls/{n}/requested_reviewers`, the response omits the handle, the timeline shows it) — pr-lifecycle §5's "the bot cannot request Copilot" grounds are stale | Facts for the front door's expected-set doctrine and Altair's routed pr-lifecycle cure: open the pull request BEFORE the first substantive push, or open it non-draft; a code PR whose connector leg never binds takes a fresh non-draft successor at the same commit under a NEW branch name (the platform refuses a second open PR on a branch that has one; run for #109 → #113), never the REST path; when the successor draws nothing either, the connector's owner-side configuration is the item |
