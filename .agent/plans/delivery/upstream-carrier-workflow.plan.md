...
| Round four, **Copilot and Codex, same finding**: the procedure closed the stale carrier before updating the mirror, so a scheduled carrier run in the gap could open a carrier at the old tip, and a failed mirror run left nothing to take up. | True at source. Cured by order, resting on a property of the workflow (it opens nothing while any carrier is open): the mirror first, then the close, then the dispatch; a carrier a scheduled run opened first is accepted. |
