...
```mermaid
flowchart TD
    Source["Versioned source"] -->|T: parse with attribution| Ingest["Ingest and occurrences"]
    Ingest -->|R: terms and keys| Core["Graph core"]
    Ingest -->|T: snapshot| Graph["RDF snapshot"]
    Graph -->|T: declared profile| Project["Property projection"]
    Project -->|T: query input| Query["Scoped query"]
    Query -->|T: selected facts| Result["Source-linked result"]
    Result -->|S: meaning and scope| Domain["Domain interpretation"]
    Domain -->|S: evidence authority| Source
    Query -->|R: traversal| Core
```
...
```mermaid
flowchart TD
    A["Pinned sources and corpus census"] --> B["Observation IR"]
    B --> C["Resolution and interpretation"]
    C --> D["Assertion Graph IR"]
    D --> E["Projection IRs and indexes"]
    E --> F["Bounded queries and tools"]
    F --> G["Outcomes and corrections"]
    G --> D
    G --> A
```
