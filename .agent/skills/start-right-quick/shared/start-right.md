...
`PRACTICE_AGENT_SESSION_ID_CODEX`, then the harness-native `CODEX_THREAD_ID`; the
Claude seeds count only on a Claude platform, so a seat opened from a Claude
