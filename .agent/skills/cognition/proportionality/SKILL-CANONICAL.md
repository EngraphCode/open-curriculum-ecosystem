...
|---|---|---|
| **Scope** | Is this one thing, or several wearing one name? | **Too big** → narrow; home the remainder as pointers with named carriers — including expansion BY REVIEW: a finding whose remedy would widen the artefact is a scope finding whatever the truth of its observation: the remedy takes no cure here, and the observation is verified before it is routed or rejected ([`pr-lifecycle`](../../change-custody/pr-lifecycle/SKILL-CANONICAL.md) §Phase 4). **Too small** → widen; the real work is larger than the ask implies |
