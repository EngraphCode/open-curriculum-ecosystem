...
todos:
  ...
  - id: ws1-merge-readiness-checker
    content: "WS1: a mechanical merge-readiness checker in agent-tools — `pnpm agent-tools:pr-merge-readiness <pr>` (exact CLI name TBD at build). Given a PR number it reports and exits non-zero unless ALL hold: (a) the PR is POSITIVELY mergeable — GraphQL `mergeable === MERGEABLE` (not merely `!== CONFLICTING`, which wrongly passes `UNKNOWN`); since GitHub computes mergeability asynchronously, treat `UNKNOWN` as not-yet-ready and poll/retry a few times before failing, rather than passing it; (b) every required check is terminal and green (no pending/failed); (c) every review thread is resolved (GraphQL `reviewThreads.isResolved`) OR carries a reply (from any author — a human or another author may refute or resolve a comment, not only the merging agent; this matches the Risks clause); AND (d) readiness binds to the pr-lifecycle skill's review-round state machine (`.agent/skills/change-custody/pr-lifecycle/SKILL-CANONICAL.md`), NOT green-checks-and-resolved-threads alone: the expected reviewer legs (item 3), the round SETTLED for the current tip (item 4), a zero body-finding tally across threads AND review bodies (item 2), and late-harvest handling of out-of-order/older-tip reviews — bound BY REFERENCE, the skill holding the single definition and this checker consuming it, never restating the mechanics here. REST `/pulls/{n}/comments` does NOT carry thread resolved-state — use the GraphQL `reviewThreads` connection. TDD; pure core (verdict function over fetched thread/check state) + thin gh/GraphQL I/O adapter; no global state. Output names each blocking thread/check so the agent knows exactly what to resolve."
    status: pending
    depends_on: []
  - id: ws2-oak-pr-skill
  ...
  - id: ws5-lifecycle-conventions
    ...
    status: pending
    depends_on: [ws2-oak-pr-skill]
  - id: ws6-pr-watch-compound-floor
    content: "WS6: extend `pnpm agent-tools:pr-watch` to surface the compound-read floor it currently omits — review bodies AND per-reviewer tip-binding — so the Phase 7 round-owed / reviewer-leg gate becomes machine-checkable rather than depending on the manual Phase 3 GraphQL harvest on every wake. Today pr-watch reads only PR-view fields, REST review comments, and thread counts, so a summary-only review carrying findings in its body does NOT change its snapshot and an OWED reviewer leg is invisible to it. Compute the reviewer-leg gate predicate as the pr-lifecycle skill's reviewer-leg SATISFIED definition (state-machine item 3, ~L267-274) requires: SATISFIED holds when ANY review in the FULL paginated reviews harvest binds (reviewer, current tip) — so harvest ALL reviews (page the `reviews` connection to exhaustion, retaining each review's `commit.oid`) and match on (reviewer, tip) for the gate. The per-author latest pointer is NOT the gate source: per the compound read's caveat (item 1) it can point BACKWARDS when an older-tip review job completes after a newer push, leaving a satisfied leg falsely OWED. Reserve `latestReviews(first:20){pageInfo{hasNextPage endCursor} nodes{author{login} commit{oid} state submittedAt body}}` (paged while `pageInfo.hasNextPage` is true) for its HINT/MARKER role — surfacing review-body arrivals and skip markers — not for deciding SATISFIED. Emit review-body arrivals and per-reviewer tip-binding as watcher state changes."
    status: pending
    depends_on: [ws1-merge-readiness-checker]
isProject: false
---
