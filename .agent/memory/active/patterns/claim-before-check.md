...
## Three variants the basic tripwire does not catch
...
- **Narrated, not performed.** The sentence "I am doing X" is generated by the
  same process as "X is done", and from the inside the two are near
  indistinguishable — so a declared pause was not a pause and a declared wait
  was not a check. Three letters name the mechanism in their own words without
  a shared rule to cite: a seat that kept announcing instead of doing
  (2026-08-17); a declared pause with questions unread beneath it, caught by
  the owner's "What are you doing?" (2026-09-02); an announced wait for the
  owner's word that was itself the wrong move (2026-09-03). Intending harder
  does not catch it; the tool call goes before the sentence, or the sentence
  is not written.
