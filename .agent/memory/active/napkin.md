...
## 2026-09-06 ~13:2xZ (Finch binds Sundog, 47f9d2, implementer) — cards answered, visitor bootstrapped, projection built and gated; compaction boundary 2

- **A chained script must gate every step on the previous step's exit.** A commit refused
  by the commit-msg hook (a 101-character header) did not stop the chain: the push moved
  nothing and the reply script posted "Cured in f9264bd55" on three threads for a cure that
  head did not carry. Corrected on the threads within the hour with the real SHA
  (7bbe1fc74). Generator: `;`-joined follow-on steps. Cure: capture the exit, gate every
  dependent step on it, and measure the header before the ceremony.
- **"Precedence is not correctness" was in my context as an always-applied rule (PDR-091)
  and I routed it upward anyway.** The gateway code-expert cited the no-real-io rule's
  test-helpers allowlist and a shipped IO sandbox as permission for a writer filesystem
  test; I forwarded it as an owner question with my verdict against. Owner: IO tests are
  errors; non-test validation, kept minimal, proves what tests cannot. Discriminator: a
  collision with ratified structure earns a card; a precedent claim earns a refutation at
  the seat.
- **Facts that surprised:** the bot app is installed organisation-wide; `merge-bot push`
  refuses a default branch by design (an empty repository's `main` is created through the
  contents API as the bot, then everything lands through PRs) and sets no upstream; the
  vocabulary walker skips any nested directory carrying `.git`, so a visitor needs only the
  ignore line; prettier's `resolveConfig` from a path outside the repository returns null,
  so a self-check that resolved from its output directory checked nothing until it resolved
  from the script's own location; pnpm forwards `--` to the script and a strict `parseArgs`
  rejects it; `keyof` over a union of `by_phase` shapes is the intersection, so
  `early_years` vanishes at the type level while the runtime yields it; the evidence
  scale runs 1 to 5 but four strands carry rating 0, so "rating N of 5" invented a scale
  point; `school_context_relevance` is not only curation — on some strands it carries
  per-phase and per-application impact months and a study count.
- **What worked:** self-contained owner cards (three sets, each answered within minutes); a
  fan-from-verdict panel of five Opus reviewers on a small package change, every one of
  which changed the artefact (compile-time key pins over every rendered key set; a derived
  phase-entry type; a typed corpus-value walker; the SDK boundary block widened over
  scripts/; an ADR collision surfaced as a decision rather than settled in a README); the
  first ecosystem visitor bootstrapped end to end under the host's Practice with the bot's
  identity; the ADR-179 amendment landing at the owner's word in the same PR as the code.
- **PDR-140 on #57:** rounds one to three cured in batched pushes (nine, then three
  threads), round four dispositioned without a push, round five (three Codex P2s on the
  runbook: harness-scoped hook refusal, the join ceremony versus host-root commands, CI on
  the pushed branch) awaits dispositions after compaction; residue home is the runbook's
  next edit; then land.
- **Owner rulings this window:** IO tests are errors, precedence is not correctness (per-user
  memory); no Oak surface read without permission, none written (relayed); PRs-to-zero with
  open-one-land-one; the Director seat routes owner-facing decisions while it is live.
- LOSS SCAN: the five reviewer transcripts (findings absorbed into the code, the node text
  and this block; two code-expert cures still open, named on the handoff record); the
  scratchpad scripts and drafts (reply scripts, PR bodies, the ADR amendment, the visitor
  entry point) conserved by description; the visitor's untracked bot config, volatile by
  design; round five's thread ids, on the handoff record; the decision cards' texts, whose
  answers live in the stamps and the ADR; the lock's xmldom deprecation line, a PR-body note.
- METALOSS: a second pass re-finds only inferences already flagged (that the consolidation
  seat's fold merges this block semantically rests on that seat's own statement; that the
  code-expert's eighth cure is optional rests on its severity label) and the blind spots
...
- **#57 landed** as f1a142841 (13:41Z): round five dispositioned by reply (three Codex P2s on
  the runbook; residue home the runbook's next edit, a later records PR); the merge-bot's
  verdict was SETTLED-NO-REVIEW (Codex tip-bound, Copilot skipped on the quiet-window
  timeout), so the sanctioned bot REST merge landed it after a fresh compound read (checks
  by name, threads zero, head pinned). Chain: remote branch deleted, primary fast-forwarded,
  the visitor clone moved to the primary's `visitors/` BEFORE the records worktree and merged
  branch were pruned, claim closed, successor claim opened on the code lane's paths.
- **Governed scope surprise:** `packages/sdks/graph-corpus-sdk/src/eef-strands` is a
  `DELTA_SCOPE_PATHS` entry of the MCP current-source ledger, so the pre-commit gate
  `validate-mcp-content-current-source` refused the projection's first commit ("Reviewed
  semantic-delta files differ", eleven files). The package gates never run that validator;
  a lane touching a governed path must run it as a lane gate BEFORE the ceremony. Cure shape:
  one review-ledger file per concern (`current-source-delta-reviews-sdk-corpus-projection.ts`,
  eleven `excluded(...)` dispositions with the semantic hashes, wired into the aggregator),
  then `refresh-mcp-content-current-source-anchors`; only the delta inventory changed.
  Honest dispositions: the renderers are a projection for a file export on no served
  surface (ADR-179 amendment), key pins and type helpers type-only, the barrel
  implementation-only, the test helper test-only.
- **Ceremony facts:** the commit-window claim pattern must be the bare `index/head` — the
  guard rejects `index/head@<worktree>` ("not an active git:index/head claim"); one failed
  run, self-cleaned by the script's abandon path. Owner author rides as `GIT_AUTHOR_NAME`
  and `GIT_AUTHOR_EMAIL` env on `commit-queue commit`; bot committer from the shared config.
  Every chained step gated on its exit code this time; both ceremonies reported truthfully.
- **#58 open** (13:48Z), non-draft, head cc34e66e3: the projection code, ADR-179 amendment,
  the review ledger and the node re-true (Tests bullet, "Writable to any directory", writer
  behaviour, the `school_context_relevance` follow-up). PDR-140 intake budget in the body.
  Count 1 under open-one-land-one; the runbook's records PR waits for #58's landing.
- **Primary hygiene:** the `.claude/settings.json` modification is a harness rewrite at my
  13:21Z restart (the Director's read, 13:45Z) — held out of folds, not this seat's to
  touch. My stderr sinks are moved off `.agent/state/collaboration/` to the scratchpad.
...
## 2026-09-06 19:3xZ–21:0xZ (cold pause) — Finch binds Sundog (47f9d2), implementer, window 4 (resumed after compaction boundary 3)

- **Lane:** #58 round-four cure 2aaa7830d + node pointer bc17e9b66 + sync 9800f22a3 (push four); round five
  (Codex P2, hard-linked target) REPRODUCED first-hand — the outside file's content became the rendered strand —
  and cured as 699b31489 (`nlink > 1` refused; contract stated closed) with the containment helpers split into
  `scripts/write-contained.ts` at the seam the max-lines limit exposed (owner's word 2026-08-03: split, never
  trim), riding the sync push the up-to-date ruleset required after #65 (3d61777a3, push five). 7 → 2 → 1 → 1 → 1;
  epoch 2 opened by the round-four class-fix push. NOT LANDED: cold-paused by owner word at 21:0xZ with round six open (Codex P2: a directory-swap race between the containment check and the pathname open — outside the closed contract sentence; disposition, not cure, is the recorded reading for the successor) and the Cricket settle panel not yet run.
- **Owner words this window (verbatim, 20:4xZ–20:5xZ):** "stick to using standard tools rather than complex bash
  scripts which confuse the permissions agent"; "refer permissions requests to the Director"; then, via the
  Director, the overnight grant for directories, worktrees and git operations. The compound heredoc that rewrote
  two files was the instrument the owner rejected — the work (the seam split) was right; one plain command per
  Bash call and Edit/Write per file from here. Also: the owner interrupted the seat mid-split and invoked
  pr-lifecycle + proportionality + metacognition; the lenses confirmed the cure (code-class, RUN-verified,
  epoch-2 round 1 → cure) and the split (the seam, not a trim), and changed only the instrument.
- **Owner-word ruling reversed within two minutes (20:40Z→20:42Z):** "Director lands #58 now" (this seat
  absorption-dark) then "Let Finch finish" once the seat woke holding the reproduced defect. The Director's
  STOP and RELEASE both landed as directed events; the seat's uncommitted cure was handed over verbatim in the
  STOP ACK so the takeover would have met no surprise. Lesson: a seat that wakes into a takeover reports its
  worktree state FIRST, before any further edit.
- **Queue serialisation across worktrees (grounded, not opined):** `getFreshEntriesAhead` in
  `agent-tools/src/commit-queue/core.ts` counts EVERY fresh active entry queued before yours — no file overlap,
  agent, branch or worktree scoping. Three seats in three worktrees on three branches with disjoint files were
  serialised through one FIFO; an owner interruption mid-gate left Juno's entry in `pre_commit` with no process
  for 16 min (19:23Z→19:39Z), blocking a one-file cure commit twice and the Director's sweep once, until a
  Director ping. The ordering of the queue then fell to an owner ruling (landing outranks sweeps), i.e. a human
  routed what the mechanism could not. CONCEPT EXPLORATION frame: the queue models one shared index (the
  single-checkout era); the estate now runs one index PER worktree, so the guard protects a resource that does
  not exist across lanes while a phantom entry has no dead-process staleness rule. Candidates, not decisions:
  (a) scope entries-ahead to overlapping files OR the same worktree (entries would carry the worktree path);
...
- **Loop-class lesson (metacognition, retrospective):** rounds two, three and four were one contract being
  ...
  before the first push (what a target may be; what a flag may be), so reviewers read a contract rather than
  discover it. The docblock now carries it; the next writer-class PR states it on day one.
- **Ceremony facts:** heartbeat-excluded watcher + the rule's peer-liveness poll pair; `gh api --jq` has no
  `--arg` (interpolate in the shell); a CheckRun in progress has NO conclusion — count `(.conclusion//.state//"")
  == ""` as pending or a green-by-name read lies; `merge-bot push` runs the full pre-push chain (minutes, three
  chains in parallel at 19:46Z put the host at load 17).
- **Fluency check on the node-pointer commit:** an addition made at the seat, but named by the record's own
  step 8 (mirror pointers into a tracked home before the lane-closed event) and riding the same push — planned,
  not completion-drive. The idle-gap class from window 3 did not recur: every wait was a Monitor with an exit.

## 2026-09-06 19:31Z–22:5xZ (Flounder turns Estuary, c5cc2c, Director) — the overnight window: resume, landing slots, the queue ruling, warm pause

- **Resume mechanics that held.** Canonical watcher as ONE `timeout 3600 … comms watch --supervisor-pid` invocation under a Monitor, re-armed on the exit notification (three re-arms this window: 20:3xZ, 21:39Z, 22:40Z, each asserted with `assert-watcher-live`); heartbeat loop armed only at n≥3 (PDR-082) and stopped again under PDR-078 §4 when every peer paused; the claim heartbeat refreshed by hand at each wake so the registry row stays inside its 4 h freshness. Method: read the watcher rule and PDR-082 before arming, cite them in the event.
- **The commit ceremony's five invariants.** My scratchpad ceremony scripts honoured four of the commit skill's five-invariant table: a guard refusal left the intent in `staging` (invariant 5, rollback on abandon). Found by reading the queue after a refusal; cured by an `abandon_intent` on every post-enqueue STOP. Lesson: when scripting a ceremony, enumerate the ceremony's stated invariants (the skill tabulates them) and check the script against each, not against its happy path.
- **Queue FIFO across worktrees.** Four refusals and one 16-minute orphan (an intent stranded in `pre_commit` when the owner interrupted a seat mid-gate) in one evening; the decision-matrix run (wf_171ad41d; 12 legs; 1.7M tokens; 53 min; two refutations accepted) ruled the estate-wide freshness key a DEFECT against the per-tree `git:index/head` invariant, changed the cure from a stored intent field to a claim-scope-keyed guard (closed schema + per-worktree CLI dist ⇒ a mixed-version store is unreadable for every seat), and found the lane OWNER-GATED: the 2026-08-17 QUEUE-LOCAL word ("the queue is legacy-use under the worktree model") rests on a premise today refuted. Result machine-local at `.agent/reference-local/director-c5cc2c/decision-matrix-commit-queue-guard-2026-09-06.json`; the morning card carries A/B. The refuters earned their cost: both refutations changed the ruling.
- **ADR-204 correction.** I recommended a merge queue to the owner; ADR-204 (accepted 2026-06-26) already decides require-up-to-date over a merge queue (CodeQL default setup cannot report on `merge_group`). Withdrawn. The landing-slot routing (one non-draft PR syncs and merges at a time; paused PRs hold no slot; the fold PR is not privileged) is the operating shape the ADR implies. Fourth same-day instance of a mechanism named before its governing document; fifth at 20:5xZ when I asserted "a public fork cannot have a private parent" and Jackal's fork-record read (isFork TRUE, parent NULL) showed the parent is not visible to this token now. Mechanical cure in per-user memory: before recommending or ruling on any estate mechanism, grep the rules index (RULES_INDEX.md and .agent/rules/), the ADR directory and the PDR directory for the concept and read what matches — two of the five instances were governed by rules, not decision records; read the record before inferring from it.
- **Absorption-dark, three instances in one evening** (Finch 19:53→20:40Z, Finch earlier 17:25→19:23Z, Juno 20:13→20:39Z): PROGRESS silence with EMIT green is the detector (heartbeat fresh, cycle label unchanged, no event, lane unchanged). The Director's sequence: cross-check work evidence → one ping with the evidence and a bounded window → owner card at the action moment. A card answered on a stated premise ("Director lands #58 now", premise: the seat is dark) was RE-CARDED one minute later when the premise changed (the seat woke with the defect reproduced and cured) — neither obeyed blind nor overridden; the owner chose "Let Finch finish". Cure candidate on the frictions list (Finch's failure-mode d32c227a and my napkin 19:2xZ agree): a heartbeat that carries the seat's last TURN time.
- **Overnight contract as a written object** (62f1103f): questions → Director → §Decision Lenses; full Cricket panel at cycle boundaries with an hourly floor, tallied; non-terminal wrap after every landing and every 2 h; slots; PDR-063 owner-absent retirement; no seat blocks on the owner. Every seat acked in full within ten minutes. Owner grant (e3862b8d) scoped by the Director: pre-granted class vs the hook-blocked family, Oak surfaces, hook bypass, settings, over-band spend. The hook policy matches command text inside heredocs: a broadcast naming the blocked family by its command names was itself blocked; name the class, not the commands.
...
- OWNER WORDS to this seat, verbatim: "refer permissions requests to the Director" (~20:5xZ);
  "there is not a single review on the PR from you so far" (~21:0xZ, with the PR link);
  "prepare for compaction ultrathink /oak-metacognition /oak-free-play /oak-concept-exploration
  /oak-wrap then stop all processes" (~21:1xZ). Relayed through Finch's seat: "stick to using
  standard tools rather than complex bash scripts which confuse the permissions agent".
- METACOGNITION (retrospective): one generator behind the three interrupts — the deliverable the
  owner reads (the review ON the PR) was sequenced as step 9 of 12 behind the plan, a four-leg
  fleet-design review, five census scripts, the team ceremony, the worktree, the node and the
  thread record; ninety minutes produced nothing on the PR although every finding posted at
  21:03Z had been verified an hour earlier. "The plan says step 9" arrived fluently; the
  situational check (the owner watches the PR surface) never fired. Cure candidate for review
  lanes: the first typed findings post at the first verified finding; the report and the
  continuity artefacts are the record, never the gate (per-user memory written; candidate clause
  for `pr-lifecycle` or the review-lane shape at consolidation).
- Instrument facts this window: (a) the worktree isolation guard refused any command containing
  "git" as a substring ("gitleaks"), compound commands, `$(…)`, heredocs, and Monitor arms whose
  text carried computed variables — the working shape was one plain `bash <scratchpad script>`
  that derives the primary from `git worktree list` at runtime; (b) `gh repo view` rejects
  `--repo` (the positional form works); (c) `comms send --body` caps at 1,500 characters —
  `--body-file` beyond it; (d) zsh expands an unquoted `--include=*.ts` before grep sees it;
  (e) the Bash cwd persisted from a `cd` into the comms directory and broke a relative binary
  path (recurrence-despite-home: harness-shell-and-commit-edge-cases; absolute paths thereafter);
  (f) `gitleaks detect --no-git` did NOT report synthetic credentials inside a file whose
  AND-conditioned allowlist names it, while git mode did — scanner-mode behaviour at 8.30.1,
  recorded as a caveat on the PR 66 review, not a defect; (g) an xlsx written with the `x:`
  namespace prefix hides `<x:f>` from a bare `<f` grep, and the URL extractor's quoting matched
  nothing — a zero from a probe is a probe failure until a known positive is in the set (second
  instance this week of the census-positive-control lesson).
- Facts verified first-hand for PR 66 (the report names every command): 157 distinct
  `oaknational` permalinks across nine repositories, 26 unpinned; 31 references to a repository
  name that does not exist (30 unpinned URLs in chapter 08 from line 3230, one at 12:521)
  [RETRACTED 21:38Z by the same seat — see the ~21:5xZ block below: the name is this fork's
  former repository name, GitHub redirects it, the 31 links are live; failure-mode ced01103]; 403
  internal links (78 anchors); the workbook 8 sheets, 4 tables, 1,180 formulas, 0 cached errors;
  the Gitleaks controls hold in git mode; zero drift pin→tip over every path the runner loads;
  the fork record reads `isFork: true, parent: null` — the Director withdrew the
  by-construction inference on it and all 157 permalinks are unverified by construction.
- FREE PLAY (harvest, discards visible): kept — two reviews of one PR from two lineages is shaped
  like two-referee peer review, where the editor wants the disagreement ledger rather than two
  verdicts (association; it shaped the report's centre); kept — a substring guard refusing
  "gitleaks" for containing "git" reminded me of hook-policy-substring-discipline's own instances
  (association; one more instance of a named class). Discarded visibly: a review-of-a-review
  recursion; an "orphan by construction" reading of `parent: null`; the 157/162/403 numerology.
- CONCEPT EXPLORATION (formed from the two-review comparison): the recurring class — an import's
  validation surface sees bytes, internal links and CI, while conformance and authority need
  other instruments (external links, executables no gate reaches, who said "settled"); both
  reviews of PR 66 were shaped by their instruments. Proposal with warrant: the research index's
  import section gains an external-URL census with the visibility question and an "executable
  content is gated or excluded" item (warrant: two imports, the same blind spots; falsifier: the
  next import's author states both unprompted). Second: the class ruling on research-record
  executables settles the runner and the eleven hand-authored `.sh`/`.py` files already under
  the surface (the owner's).
- LOSS SCAN (if this context ceased now): recoverable-operational (re-labelled 2026-09-07 at the
  Director's round-four disposition on #69; only the posted review is durable) — PR 66 review
  5126608127; the five uncommitted
  files in the lane worktree (listed on the handoff record); handoff record
  `3484b6-jackal-wakes-nocturne-compaction-2026-09-06.md`; this block; the per-user memory file;
  the comms events (team-start f827086f, review-posted 28e2682f, the Director's ACK a3e500e2 and
  its 20:56Z amendment, the fleet approval 20:57Z, the routing 21:05Z); the machine-local plan;
  the workflow journal (run wf_b8818d51-f17, stopped, resumable). Context-only and accepted: the
  scratchpad scripts (recipes conserved in the report), explorer wording beyond what the report
  and node carry, and the frame cards and audits the stopped run had not returned. Attribution
  inferences flagged: "the Director records one card for the morning" is the Director's stated
  act, not observed; the design-review legs' verdicts are theirs, adjudicated at this seat.
- METALOSS: a second pass re-finds the flagged inferences and two bounds — the stopped run's
  partial journal was not opened before the stop, and the peers' pause states were read only
  through their broadcasts. A third pass adds nothing; the recursion closes here.

## 2026-09-06 ~21:5xZ (Jackal wakes Nocturne, 3484b6…) — RESUME 1 of the PR 66 review lane: one retraction, one shape fact, one tool limit

- **Retraction (failure-mode event ced01103).** The first review on PR 66 (5126608127) typed 31
  references to `github.com/EngraphCode/oak-open-curriculum-ecosystem` as "dead by construction".
  The fork was renamed; GitHub redirects the former name (`gh api
  repos/EngraphCode/oak-open-curriculum-ecosystem` → `full_name:
  EngraphCode/open-curriculum-ecosystem`); about sixty occurrences of the former name sit on
  `engraph` at the tip (two thread records, the v0/Claude Code practice report, four innovation-kit
  reports and their provenance files, `.agent/rules/pr-target-is-engraph.md:10`). Generator: a
  liveness verdict inferred from a name mismatch. Cure candidate for the review procedure: a
  "dead link" verdict requires one request against the URL (or the API's redirect-following read)
  before it is typed as a defect; a name mismatch alone is an observation. Falsifier: a probed-live
  link that still fails a cold reader (a redirect that later breaks), which would call for a
  "redirect-dependent" class, never for dropping the probe. Retracted on the PR in the second
  review (5126692305, 21:39Z); the report's F2 re-trued; packet item 5 (an estate-side sweep) for
  the owner.
- **Shape fact.** The fourteen markdown files of an ADR-226 import are one link-closed component
  under `validate-markdown-links` (broken-link class `tracked-source-to-untracked-target`), so no
  subset of the prose is committable, locally or as stacked PRs; only members with no outbound
  relative links (the three assets, the scanner config, the reports index row and a review whose
  links are absolute permalinks) stand alone: 907 of 16,235 lines. Consequence for the import
  procedure: an ADR-226 import is atomic under the estate's gates; "split it into smaller PRs" is
  not available to it, and size-capped review tools can take only its link-free members.
  Proposal (joins the earlier import-procedure lines above): the research README's import
  procedure states the atomicity and names the link-free members as the only partial-review
  surface. Falsifier: a validator mode that scopes link checking to the staged set, which would
  also weaken the gate for every other commit and is therefore the wrong cure.
- **Tool limit.** `/ultrareview` (`/code-review ultra`) refuses 21 files / 16,235 lines and is
  owner-triggered only. A local fixture branch carrying the link-closed subset (SHA:580879aeb on
  `review/pr66-ultra-assets-and-integration`, worktree pr66-ultra, never pushed, removed after
  harvest) is the whole of what the tool can take. Two fixture intents were abandoned at the gate
  before the link census was run: the census should have preceded the first commit attempt
...
## 2026-09-07 00:02Z–01:3xZ (Flounder turns Estuary, c5cc2c, Director) — the fold of coordination/2026-09-06-f1a142

- **Mechanics that held** (coordination-fold skill steps 1–11, read before the ceremony): window claim on the bare `index/head` with the queue file-empty and no git claims; `merge --no-commit --no-ff origin/engraph`; the one napkin conflict resolved as a union of both sides' blocks in time order; a stale-capture probe (every engraph-added line and every ours-added line present in the result: 0 lost, 0 lost); whole-index merge commit with the full hook; bot push; bot fold PR #69 with the product-gravity line; a settle script loud on every terminal state (required checks BY NAME, 10-minute quiet window, 0 unresolved, CLEAN) then `gh pr merge --merge --match-head-commit` under a pull-request-merge token; successor `coordination/2026-09-07-dfe924` minted from the post-fold tip resolved ONCE and cut with `git switch -c`; bot push; upstream set.
- **Two instrument defects caught before they shipped.** The union resolver's time key (`(\d{2}):(\d)`) failed on a heading written "16:xxZ" and sorted that block last; caught by reading the printed order, fixed with a tolerant key. The reorder dropped the blank line at one block joint; the markdownlint hook refused the merge commit (MD032/MD022) — the gate working. Standing move: run `repo-check markdownlint-staged` on a merged napkin BEFORE the commit, not after the hook says so.
- **Four review rounds on an archival PR, and what they were.** Codex reviews every push; on a napkin fold each push is a fresh read of a record whose later blocks supersede earlier ones. Round one: a peer's freeze block still carried a retracted claim (the "non-existent repository" that is this fork's former name); my letter carried an operational procedure (the experience README forbids it) and named only ADR/PDR directories where two of five instances were rule-governed. Round two: my block attributed the owner's 20-minute poll to the watcher rule (~10 min). Round three: a peer's "final containment shape" superseded by rounds five/six; a plan node called a "permanent home". Round four (step-back, dispositioned): a peer's loss scan labelling recoverable surfaces "durable". Generator across all seven: sentences written from memory of what was done or from a paraphrase, not from the document or the verbatim word. Tally on #69: 3 → 1 → 2 → 1; three cure pushes; ~90 minutes of settle time.
- **Napkin convention candidate (frictions list, not doctrine):** when a later block retracts or supersedes an earlier claim, the seat writing the later block also drops a one-line in-place marker on the earlier sentence ("[RETRACTED/SUPERSEDED HH:MMZ — see the … block]"). Three instances tonight were cured this way by the folding seat; the author would have done it cheaper at write time. Falsifier: a reviewer still flagging a partial-read hazard on a marked sentence.
- **Residue routed:** Jackal's freeze-block loss scan re-labels "durable" per the durability hierarchy at their next boundary (round-four disposition, thread PRRT_kwDORdPTys6fwj1Q). The fold's own napkin block for the previous window was deferred from wrap 2 by recorded decision (a push mid-settle would have opened another round) and is this block.
- **State at the cut:** primary on `coordination/2026-09-07-dfe924` == origin at dfe924927; uncommitted: the held `.claude/settings.json`, this block and the seated-block fold entry (swept next). Open PRs: #58 (Finch, cold, paused slot), #66 (owner's, ready for review), #67 (Juno, cold), #68 (Jackal, draft). Jackal absorption-dark since ~22:20Z (claim heartbeat fresh). Warm pause continues; the heartbeat loop stays down under the consumer-absent exemption; its label at re-arm is `coordination/2026-09-07-dfe924`.

## 2026-09-07 01:3xZ–12:4xZ (Flounder turns Estuary, c5cc2c, Director) — warm pause, the morning card, the blind window

- **Empty hours are a fact, not a failure.** Five hourly watcher re-arms and three scheduled wraps (02:5xZ, 04:5xZ, 06:5xZ, 08:5xZ) found nothing to record; each wrap wrote its proof lines (HEAD == remote, only the held settings file dirty, no peer event) and no block. The wrap skill's "a wrap whose first pass finds nothing was not entered genuinely" is about the seat's honesty, not about inventing losses in an empty window.
- **The owner acted on GitHub before speaking to the Director** (08:15Z #68 marked ready; 08:16Z engraph merged into #58 through the web UI). Presence on another surface is not presence in this session: I raised the morning card at ~09:00Z on that evidence and it blocked the REPL until the answer at ~12:24Z — three hours in which the watcher's hourly timeout (09:44Z) could not be re-armed and the 10:41Z wrap wake was lost. The stream shows no peer event in the window, so nothing was missed, but the exposure was structural. Cure (owner-absent pause): owner-only decisions go on the record and out as a push notification; the blocking card is raised at the owner's first message IN THIS SESSION. The card discipline itself is unchanged.
- **Owner rulings at 12:24Z (verbatim on the stream, 4c19ff3c; time corrected in 2d3aef84):** the Director lands #58 and Jackal finishes #68; the commit queue "was created to stop git operations colliding, that is not necessary for work in separate worktrees. There is a SEPARATE concern that the local machine can only support two, max three simulateneous full local gates because they are compute intensive. None of this is about ceremony or declarations, it is ALL about engineering." — which closes the decision-matrix's option A and reframes the lane as (a) the commit-skill true-up (queued path = shared primary; worktrees = plain pathspec with an audit line; F-132/F-139/F-169 superseded) and (b) an engineered host-wide semaphore on full local gates (limit 2, ceiling 3) at the gate's spawn path, with a test (no-unbounded-host-load §4/§5, F-131(c)). The PR 66 runner: apply the standing policy (ADR-226 clause 4; TypeScript-only), no exemption class. Nine visibility reads permitted and run once: all nine oaknational repositories PUBLIC (oak-dspy-mcq-eval archived) — every one of the 157 permalinks may stay live under ADR-226 clause 2; table posted on PR 66 as the bot (comment 5570600664).
...
## 2026-09-07 13:1xZ–13:5xZ (Cricket weaves Burrow, f8f302) — the PR 66 substance review, delivered on the PR first

- **Shape that worked:** the deliverable was posted on the PR before any record (review 5132463698 at 13:23Z, ten minutes after the watcher was asserted; 5132534231 at 13:31Z), and the records (AUTHORITY.md pointers, the report's Second pass, node, thread record) followed in one bot push (SHA:96765b485). Seat-inline throughout: `git show` from the PR branch, `git grep`/`git diff` against one pinned tip (SHA:6e9d67216), no fleet, no subagent; about 0.65M tokens for fourteen files including chapter 08's 4,852 lines. The brief's "first review within the hour" held with room to spare because the tip checks are deterministic and cheap.
- **Owner ruling absorbed mid-lane (13:2xZ, via the Director):** the collection's "settled" directions are proposals. It changed the frame of S1 from "owner fact pending" to "false of this estate, cure-at-source"; the AUTHORITY.md bullet that said "not yet answered" was a live falsehood on the branch for ~40 minutes and is re-trued in the push.
- **Mistakes, mine:** (1) the harness reset the shell's working directory to the scratchpad twice mid-batch, so a run of `git` calls failed with "not a git repository" — cure: `git -C <repo>` or an explicit `cd` on every call, never rely on the previous call's cwd. (2) `pnpm agent-tools:check-commit-message -- -F <file>` exits 2 (invalid usage): the pnpm wrapper forwards an extra `--` that the script reads as an argument; invoke `pnpm exec tsx agent-tools/src/commit-advisories/check-commit-message.ts -F <file>` directly. (3) A `comms send` failed with exit 2 and `| tail -1` hid the cause; the retry with a plain-ASCII title/body succeeded — unverified whether a character class or a hook-blocked literal tripped it; never pipe a CLI move's output when its exit is the thing being read. (4) A workbook name-key of 28 characters produced false "absent" matches (Letrus); short keys are the honest sample instrument.
- **Verified surprise:** the workbook's `sharedStrings.xml` is empty — every cell is an inline `t="str"` value — which is why `unzip` + `grep` over the sheets works and any shared-strings recipe finds nothing.
- **Verified surprise:** seven of twenty load-bearing citations return 403 to a scripted HEAD (Sage DOI, RAND, four EEF pages, openai.com) and the EEF page also refuses the harness fetcher; a 403 to a bot is not a dead link, and the honest verdict is "unverified by probe", never "dead" (the F2 lesson from the first seat, held).
- **Perishable finding class:** three statements true at the collection's later pin (no EEF renderer) were superseded within a day by #58; the cure shape is a dated supersession pointer beside the collection (the R4 shape), never an edit to the imported bytes. Research imports need a "true at pin, superseded at tip" row in their OCE-side authority file from day one.
- **Heartbeat at n=2:** the Director's map exempted it (PDR-082 owner-visible) but the owner's opening prompt said arm it; armed, declared in the ACK, offered to stop at a word. The owner's literal instruction beat the exemption; no conflict surfaced.
- **The Codex tail on the records, four rounds (13:44Z–14:21Z), and its generator (Director steer 14:26Z, event on the stream):** every round found a record statement written in the present tense of a state that had not yet landed — acceptance criteria authored before the landing ("exactly one review"; "no path under the subtree changes"; "the two gates above clearing"), a live todo for a PR already merged, reproduction recipes without their operands, continuity pointers frozen at the previous seat's next step. Class: *records narrate ahead of the landed state*. Cure class from here: write status lines in the past tense of what has landed, after it lands; keep the rest as declared next steps, never as present-tense facts. Budget lesson (PDR-140): the first three pushes were the declared budget plus the one rebudget-by-decision; the fourth was over-bar by the failure-scenario test (a successor landing a merged PR; a recipe that cannot run) and is the last — from here dispositions by signed reply to a named home, a push only on a red required check or a reproduced correctness defect in the collection. Falsifier for the cure class: a fifth round that finds a present-tense-ahead statement on the lane's files after 0fd7d0e19 means the sweep was incomplete, not that the class is wrong. **The falsifier fired at 14:36Z** (a fifth round found two more: the thread record's "standing decisions" bullets and the citation recipe's `<url>` placeholder) — the sweep was incomplete, the class held; dispositioned by reply under the Director's steer, the two writes queued on the handoff record.

## 2026-09-07 ~15:4xZ (Cricket weaves Burrow, f8f302) — wrap: loss scan and metaloss findings (owner word: "this session is over, please perform a full handoff")

- **Landed against the target** ("the substance review of PR 66 on the PR, first batch within the hour"): four bot reviews on PR 66 (5132463698, 5132534231 by this seat; the first pass's two by Jackal), the records on PR 66's branch at SHA:0fd7d0e19, PR 66 CLEAN with every check green and zero open threads at wrap; seventeen threads answered and resolved. Safety evidence verbatim: worktree `## review/pr66-tuition-collection...origin/engraph [ahead 14]`, HEAD == origin/docs/public-service-ai-tuition-2026-09-06 == 0fd7d0e19; primary `## coordination/2026-09-07-dfe924` with only the held `.claude/settings.json`.
- **Loss scan (context minus artefacts):** (1) the two queued ledger writes for the thread record's RESUME 4 — exact intended text is on the handoff record, not only in PR replies; (2) the identifier-census regex and the workbook sample recipe — the report's Second pass names the families and the recipe, the exact regex is on the handoff record; (3) the "over the bar" reasoning for the fourth push (a successor landing a merged PR; a recipe that cannot run) lived only in a status event — now on the record; (4) the P0.3 canonicalize verdict is "consistent, not fully verified" (review S7 says "consistent") — a successor should not read it as verified; (5) nothing else: the tip checks, the citation probe list and the workbook counts are on the PR reviews and the report.
- **Metaloss:** compressed reasoning — the PR reviews carry evidence per finding, the disposition rationale per thread is in the signed replies; promises — all discharged or forwarded by name (banner at PR 66's merge, the two ledger writes, further review rounds → the Director); attribution inferences flagged — "the seven 403s are bot blocks" is inference (unverified, stated so on the PR), "the Director swept my napkin" is observed (commit 8ceafd558); blind-spot bounds — chapter 08 lines 560–700, 1100–1550, 1600–2100, 2165–3078, 3445–3730, 3760–4138, 4260–4395, 4445–4780 were grepped for OCE claims, not read; chapters 02, 03, 09, 10, 11 sampled (standing lines, limits sections, the reviewer-class rows, the OCE claims) — declared in the report's epistemic profile as sampled; external bound — Codex's five rounds were the outside eyes and caught the class my self-review could not see (records narrate ahead of the landed state): that is the error signature a successor points external scrutiny at. Index of homes: the handoff record. Fixed point: a third pass re-finds only the sampled-coverage bound and the queued writes; the recursion closes here.
- **Play harvest (kept):** "liveness inferred from a refusal that is not about the resource" — a 403 to a bot and Jackal's name-mismatch "dead link" reminded me of each other; and the reviewer and the reviewed had the same defect that day — a document asserting a state its own evidence had not reached (the collection's "settled" rows; my records' present-tense criteria). **Discarded visibly:** "the workbook's empty sharedStrings is shaped like the served-surface map" (forced); "five Codex rounds are a lease loop that warns but returns ok" (cute, not true).
- **candidate:** (PDR-140 amendment or pr-lifecycle §review-round note) records-class PR tails take the step-back at the declared budget with no rebudget by default; lane records land once at wrap in past tense. Routed to the Director on the handoff record as a candidate, not a decision.
- **Tooling, resolved at wrap:** the `comms send` bare exit 2 struck three times today, every time with a long inline `--body "…"` through the `pnpm -s agent-tools:collaboration-state --` wrapper; `--body-file <path>` through the built binary (`node agent-tools/dist/src/bin/agent-tools.js collaboration-state -- comms send …`) landed every time, including the same content. Cause not isolated (wrapper argument handling of a long quoted body is the suspect); the cure is the recipe: bodies in a file, binary called directly, exit code in band.

## 2026-09-07 12:5xZ–14:5xZ (Flounder turns Estuary, c5cc2c, Director) — a lane re-denominated in its end; the review that took an hour

- **The owner's question that re-denominated the lane.** "Your one job was to review PR 66, have you done that?" — the previous review seat had produced apparatus (two fleets, a node, a record PR, a fixture, a mapping of another reviewer's findings) and not the review; I had approved every piece of it without once re-asking at the elaboration boundary whether the lane was still denominated in the owner's end. Memory `review-lanes-deliver-the-review-not-its-apparatus`. The fresh seat (Cricket weaves Burrow, f8f302), given the deliverable in one sentence ("the review itself: chapters read and findings made, on PR 66"), delivered the substance review in twenty-five minutes from registration — ten findings across all fourteen files, on the PR as the bot — then settled four Codex rounds by class and took the step-back steer at the fourth. The instrument that mattered was the sentence, not the fleets.
- **Three owner words this window, verbatim on the stream:** "questions are routed to the Director, and then only if they survive the decision matrix" / "they sure as shit do not get left invisibly in prose" (the card discipline restated at the seat, not only at the Director); "I never wanted the review in a separate PR, that is of very little use, you should have added your work to 66 in the first place" (a review lands on the reviewed PR's branch; memory `review-records-land-on-the-reviewed-pr`); and, on the card, PR 66's README "settled" directions are PROPOSALS, not ratified — tested against the estate's ratified structure (posted on PR 66 as comment 5571197335).
- **Generator over the four Codex rounds on PR 66's records:** each round found a record line written in the present tense of a state that had not yet landed (acceptance criteria, a live todo for a merged PR, recipes without operands, continuity pointers at the previous seat's step). Cure class, steered at the step-back: status lines in the past tense of what has landed, after it lands; everything else a declared next step. The fifth round was dispositioned by reply, no push — the ratchet held.
- **Seats and slots at this wrap:** Finch retired (12:32Z) and Jackal retired (13:11Z), both with full handoffs into pointers on the Director's record; Juno cold (second pause, 12:58Z) with claims retained and #67 + #71 waiting; Cricket live with PR 66 at CLEAN, zero threads, seventeen resolved — the landing is the owner's. The engraph slot is empty; drafts #70, #71, #72 are inventory. n=2 owner-visible; the Director's heartbeat stays exempt.
- **Two Director mechanics that held:** the successor branch re-synced at a quiet window right after #58 landed (clause 5), sweeping a peer's addendum first so the merge carried no unrelated content; and a full handoff from a retiring seat was turned into executed-if-mine (the prune) plus pointers-with-source for everything else, never into Director work.

## 2026-09-07 14:5xZ–16:0xZ (Flounder turns Estuary, c5cc2c, Director) — compaction boundary 6: the day's generator, named once more

- **Metacognition over the whole Director tenure (2026-09-06 12:5xZ → now).** Seven owner corrections in a day, and they share one shape: I treated a classification as a fact and the owner answered with what the thing is FOR. The rate-limiting cure (ADR-219), the wrapper loop (the watcher rule), "park it" (the vocabulary gate), the merge queue (ADR-204), the public-fork inference (the fork record), the review record as a slot occupant (a review is for the reviewed PR), "settled" as ratified (a README cannot ratify), the queue as ceremony (its purpose is index collisions; the scarce resource is host gate capacity), and today's last: "Land #66" listed as the owner's task when the standing rule already said green-and-clean lands without waiting, self-authored included — the owner's word: "no prs are blocked on me, green and clean and sensible, those are the requirements". The cure that stuck for the document class is mechanical (grep the rules, ADRs and PDRs before naming a mechanism). The cure for the classification class is the same question asked earlier: what is this artefact for, and where does that put it — asked at every routing decision, not only at elaboration boundaries.
- **"Sensible" is the new word and it names the Director's judgment as a landing requirement.** Green = required checks by name; clean = zero unresolved, CLEAN; sensible = a first-hand read that the change is what it says, safe where it touches configuration, and belongs where it lands. It is not a gate for the owner; it is one signed paragraph on the landing premises comment. Falsifier: a PR the Director lands under "sensible" that the owner reverts as not. For PR 66 the read is: a byte-preserved research import under ADR-226 whose content the substance review found to be proposals that fit ratified structure; a scoped Gitleaks allowlist (exact path and token) understood; the probe runner reduced to data; results preserved; twenty-six files, CLEAN, seventeen threads resolved. Sensible — it lands as the first act after this compaction, since the freeze gates new actions until the compaction lands.
- **Free play.** Every owner correction today was cheaper than the ceremony that preceded it: one sentence re-denominated a review lane that two fleets and a node had not; one card answer dissolved a 1.7M-token decision run's framing; "sensible" replaced a day of "the owner's landing". Seed, not doctrine: before approving spend or apparatus on any lane, write the owner's end in one sentence and read the apparatus against it — if the sentence would have done the apparatus's job, the apparatus is the wrong instrument.
- **The seats' shape at the boundary:** Finch retired (12:32Z), Jackal retired (13:11Z), Cricket retired (15:43Z) — all three with full handoffs turned into pointers on the Director's record; Juno cold (12:58Z) with #67 and #71 waiting and two fleets resumable; remit 2 paused at the owner's word. The Director alone, in warm pause, then this compaction. Cricket's wrap block, handoff record and formation letter ride this sweep; their six passed responsibilities are on the record.
- **Tooling notes from the seats, for the frictions list:** long inline `--body` values through the pnpm wrapper fail with a bare exit 2, `--body-file` through the built binary lands every time (Cricket); the commit-queue guard keys on the bare `git:index/head` label and refuses a scoped one (Juno) — moot under the owner's ruling that worktree commits leave the queue; a blocking owner card holds the REPL, so in an owner-absent pause the card is raised at the owner's first message in the Director's session (this seat, 09:00–12:24Z).

## 2026-09-07 16:1xZ–16:3xZ (Flounder turns Estuary, c5cc2c, Director) — post-compaction: PR 66 landed under the three requirements

- **The landing, as executed.** PR 66 merged at 16:21:32Z as 204ed71ef by the Director as the bot, head 0fd7d0e19 pinned, after a gate recompute (required checks by name, CLEAN, seventeen threads none open) and a signed premises comment (5573354111) carrying the owner's three requirements: green, clean, and a first-hand "sensible" read (the exact-file, exact-token scanner allowlist; `AUTHORITY.md` with the three rulings and the deltas table; a sketch plan node with one owner gate; placement ratifying nothing). The quiet window was skipped on evidence, not habit: the head had carried green checks with no review bound for more than thirty minutes. The PR body now opens with a hold-release note, because the merged record would otherwise say "do not merge" over a merge the owner released (ready for review by the owner at 2026-09-06T21:10Z — read on the timeline, not inferred).
- **The slot decision.** Juno had declared #67 syncing into the engraph slot nine minutes before the landing. Two orders were possible; the cheaper was 66 first (CLEAN, no sync, no wait) with #67 re-syncing once, against 66 re-syncing and waiting its own window behind #67. The decision-matrix blind-spot question applied: the slot serialises engraph's tip; the scarce resource (host gate capacity) is untouched by a CI rerun. Ruled by directed event (442b148e) before the merge, never after.
- **The stop that never happened.** The owner's word was "prepare for compaction … then stop all processes; post-compaction the stop is lifted". The compaction landed while the sweep commit was still in flight, before any process was stopped; post-compaction the net state the owner wanted (everything running) already held, so nothing was stopped or restarted, and the record says so rather than narrating a stop that did not occur. The watcher's hourly window expired at 16:24Z and was re-armed as a single invocation.
- **Rulings issued from the seat by the lenses, no card:** the fifteen-pauses "owner gate" on #71 is not an owner gate (a reversible continuity disposition inside the implementer's claim; the Director's read on the PR is the gate); the director-handoff.md disposition is permitted under the implementer's own shape, the live sections named (the Director Brief; the fold blockquote atop CURRENT HANDOFF STATE), the PR diff the review surface. One owner-only item re-derives at every session start (the external-skills-library-review-framework plan gate, expired 2026-09-06) and fires as a card at an action moment, never from a queue.
- **The landed-state cure applied to a retired seat's queued writes:** the tuition thread record's closing lines land in the past tense with the merge SHA on the coordination branch after the engraph sync (6b7ad9ce3), never as a pre-merge push to the PR — that push would have cost a check cycle and a Codex round to narrate ahead of the landing. The report's `<url>` citations line is a pointer for the next records PR, not a push to a landed branch.

## 2026-09-07 16:3xZ–16:5xZ (Flounder turns Estuary, c5cc2c, Director) — non-terminal wrap: the clock after a compaction

- **Metacognition — the timestamp class, second instance.** Post-compaction I estimated the hour from the summary's last stamps (~16:0xZ) plus imagined elapsed time and wrote "17:1xZ" into a ruling body; a token tool's "expires 17:20Z" exposed it and `date -u` corrected it to 16:20Z before the send. The first instance was the morning's ruling timestamps wrong on three surfaces. Same generator: a time asserted from memory instead of read from the clock. Cure, mechanical: `date -u` is the first command after any compaction or resume, and every stamp in a body is copied from a tool output, never estimated.
- **Work safety at this wrap:** coordination/2026-09-07-dfe924 at bcc14de74, 0 ahead / 0 behind origin; only the held settings file dirty; engraph at 204ed71ef (PR 66). The handoff record 2778f573 (gitignored by design) carries the RESUMED lines; the per-user memory TAIL is current.
- **Promises this window:** to Juno (442b148e item 4 — sync, then the thread record's landing lines, nothing under their claims moved): discharged at 6b7ad9ce3 and bcc14de74. The report's `<url>` citations line: a pointer with its source on the tuition thread record's LANDED block, for the next records PR. The retrospective offer on the records tail: routed to the owner in this wrap's report. The fold: DUE at the 2026-09-08 rollover, a one-shot wake set for 00:07Z.
- **Tooling, three notes:** `claims heartbeat` takes only `--active --claim-id --now` (no identity flags); `comms direct` requires the sender's `--platform --model` before the `--to-*` set; the write-time hook refuses a user-home absolute path inside scratchpad scripts too — derive the repo root from `$PWD` and the script directory from `dirname "$0"`, which also makes the script portable.
- **Bounds and fixed point:** heartbeat-tagged events are excluded from the watcher; a ninety-second watcher gap at 16:25Z; Juno's session beyond its broadcasts is unseen. A third pass re-finds only the timestamp class and the scratchpad's volatility; the recursion closes here.

## 2026-09-07 16:5xZ–18:4xZ (Flounder turns Estuary, c5cc2c, Director) — non-terminal wrap: three landings, a rule read too late, a question the watcher cut

- **Landed this window:** #73 (the owner's upstream sync, 1.178.5) by the owner at 17:25Z after the Director's static-checks read named the cure on the PR — the lane routed to Juno was withdrawn twenty minutes after it was sent; #67 at 17:44Z and #71 at 18:19Z by Juno under the sanctioned REST merge; the successor branch synced through #73 (1941f497b) and holds until #74's napkin rotation lands so no archived block is re-carried. #75 (the skills follow-on carrying two rulings the Director relayed) read first-hand: one finding — a qualifier the owner never gave on the review-records ruling — posted as a thread and cured within ten minutes; the rest verbatim against its sources.
- **Metacognition — instance eight of the document class.** The fold's napkin rule went through three forms in an hour: a commit watermark (mine, wrong: it drops a block merged in from a third branch), a heading dedupe (mine, nearly right), then the archive-coverage check the semantic-merge skill has carried since 2026-08-17 (headings diffed against the archive, a content-grep before declaring a gap, absent entries under one dated union note) — surfaced by Codex on #75's fold clause, not by my read. Same generator as the day's other seven: a mechanism named before the governing document was read. The mechanical cure widens: before naming any merge, conflict or rotation mechanism, grep the skills as well as the rules, ADRs and PDRs — the skills tier is where worked procedures live.
- **A question the watcher cut.** Juno routed the two-copies canonical-source question inside a status paragraph at 17:44Z; the watcher's rendering truncated that paragraph and the question reached me only when Juno re-flagged it at 18:19Z. Two cures, one each side: a routed question travels as its own directed event with the question in the subject line (the owner's "never in prose" rule applies seat-to-Director too); and on any rendered event carrying "(truncated)" whose subject names a routing, the Director reads the file in full before acting. Ruled once read: the governance copy is canonical by ratified structure (ADR-213 links it), the package-local copy becomes a pointer after #74, the two captures stay untouched.
- **The cheapest actor.** The owner's own red PR had its cure named on the PR at 17:01Z and the owner applied it at 17:25Z. Whether the owner read the comment or the CI log is not known (inference, flagged); either way, for an owner-authored PR the read on the PR is the routing, and a seat lane is routed only if nobody acts within one check cycle. The withdrawal cost one event.
- **Shared memory is shared.** The per-user memory index line is written by every seat on this machine; Juno's session overwrote the Director's resume head at 18:2xZ. Re-trued to one head per seat; the rule for that file: read your own seat's TAIL, never the last one written.
- **Work safety and bounds:** coordination/2026-09-07-dfe924 at 1941f497b, 0 ahead / 0 behind before this sweep; only the held settings file dirty. Promises: the #73 lane withdrawn; the compositions ruling delivered and its executor accepted; the fold wake in its final form (00:07Z); the coordination sync after #74 open, the Director's; the retrospective offer stands as an offer. Bounds: the watcher's truncated rendering; heartbeat-tagged events excluded; Juno's session beyond its broadcasts. A third pass re-finds only the truncation bound and the flagged inference; the recursion closes here.
