...
- **Play seeds**: a ruling outside the queue is a letter addressed to an office with no one in
  ...
  could not find it.
- **Loss-scan finding, at close (Geyser rides Pewter, eeecbd, ~12:10Z)**: O1 amends PDR-063, which lives
  under `.agent/practice-core/decision-records/`, and `directive-file-context-budget.md` gates
  `.agent/directives/` only. So O1's landing is not held by the 30% line and needs no sub-30%
  context; the retrospective's proposal 1 wording ("executor any seat below 30%") is over-strict
  for PDR clauses. For the seat that lands O1. This line rides the next napkin commit on the
  shared tree.
