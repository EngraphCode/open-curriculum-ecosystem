...
| [`review-cost-ledger.md`](review-cost-ledger.md) | The record the review cost gate is calibrated against: one row per pull request a session touched (rounds, cost against budget, the gate's verdict, the seat's stop-round reading, agreement), and every weight change | Indefinite; the gate's weights change only against it | `wrap` (the survey step); the owner corrects readings | Calibration record; subordinate to the pr-lifecycle state machine for what a round is |
