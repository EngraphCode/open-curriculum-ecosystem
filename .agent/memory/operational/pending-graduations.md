...
- **A fast machine hides a quadratic: measure growth at two sizes, never absolute time** — 2026-09-10, the Bash-guard matcher (#116): a scanner change read a growing word's last character per character, and V8's rope flattening made it quadratic; a 200 KB word took 1.4 s locally, inside vitest's five-second budget, and 6.3 s on the runner, where the PR's own test timed out red. The reviewer's timing table had shown every 200 KB shape linear on the previous head, and the cure was not re-timed. Absolute time on a fast machine shows a small constant, never linearity; timing the same operation at two input sizes and comparing the ratio (about 2 for a doubling when linear) shows the growth in ten seconds. The cure is by construction, never a raised timeout, as the no-wall-clock rule already says.
  `[captured: 2026-09-16 | source: napkin 2026-09-10 (Altair's landing block and letter "the fast machine") | target: .agent/directives/testing-strategy.md §No wall-clock ceilings in gated tests, one sentence naming the growth measurement as the benchmark instrument's method | trigger: the dedicated consolidation's directive pass, under the directive-file-context-budget rule's 30% headroom | size: two sentences | status: due]`
- **The directives tier's cheap self-standard defects** — a read-only review of all fifteen directives (Efreet guards Patina, 2026-09-10, conserved verbatim at `.agent/research/directives-tier-review-2026-09-10-efreet-findings.md`) found defects that change no doctrine; three were re-read and still stand at the 2026-09-16 tip (the template path, the "forthcoming" reference, the AGENT.md routing line) and the rest are unverified since: a dead template path in `schema-first-execution.md` (`code-generation/typegen/mcp-tools`); `tdd-as-design.md` calling `validation-strategy.md` "forthcoming" (it has existed since 2026-06-23); a dead plan citation in `definition-of-delivery.md`; `continuity-practice.md`'s stale "Last Updated" header and a dangling pattern name; `orientation.md` enumerating six of fifteen directives and misdescribing the plan estate; `AGENT.md` routing `metacognition.md` "for planning work" only; fitness frontmatter absent on four files (at the 2026-09-17 tip: `editorial-tone.md`, `validation-strategy.md`, `metacognition.md`, `cloud-environment-routing.md`); and the eight permanent-to-ephemeral citations across six files that the review's §3 item 4 counts, two of them to dead paths (the `definition-of-delivery.md` plan citation is one). Line numbers in the review were read at `064f42093`; re-verify each at the live tip before editing. Misleading docs are blocking (principles §Code Quality).
  `[captured: 2026-09-16 | source: .agent/research/directives-tier-review-2026-09-10-efreet-findings.md §7 step 1 | target: the named .agent/directives/ files, each defect cured in place | trigger: the dedicated consolidation's directive pass, under the directive-file-context-budget rule's 30% headroom | size: about twenty small edits across up to twelve files | status: due]`
- **Three Practice Core texts that contradict the live estate** — (1) PDR-008's script tables define `check` as the mutating alias of `check:fix` and name a `check:ci`, and `practice-verification.md` item 9 lists the same set, while the root `package.json` defines `check` as the read-only aggregate, `fix` as the mutating one, and no `check:ci` (verified 2026-09-12 by `jq` against a peer's report; re-verified by a locator 2026-09-16); (2) PDR-117 §Takeover verification and the Director readiness gate still treat claim freshness as evidence, where the successor's Moment 2 rests on the outgoing seat's explicit written stand-down and a stale-read row licenses nothing when fresh either (2026-09-03, Civet calls Crypt); (3) PDR-141 decision 7 says the shared grounding "runs the host's profile check FIRST", beside decision 14's pull at session open before the profile is read — start-right §3a was cured at #150 to pull, then check, then read.
  `[captured: 2026-09-16 | source: napkin 2026-09-12 (PDR-008 finding) and 2026-09-03 (Oak-line Director block); #150 round one | target: .agent/practice-core/decision-records/PDR-008-canonical-quality-gate-naming.md with practice-verification.md item 9; PDR-117 §Takeover verification; PDR-141 decision 7 — each an amendment-log entry plus the in-place text | trigger: the dedicated consolidation's Practice Core review (step 8) in a fresh context | size: three amendments | status: due]`
- **The one CodeQL exception is a permanent tracked exclusion, for that one query only** — owner, verbatim, answering which mechanism the 2026-09-08 ruling meant (a question asked 2026-09-16 ~22:1xZ; the answer read 2026-09-17 ~12:2xZ, its own time not observed): "a permanent exclusion is allowed, but ONLY for that one issue". The issue is `js/missing-rate-limiting` on the MCP server's routes, and the mechanism is the tracked `query-filters` exclude already in `.github/codeql/codeql-config.yml` (`bd7a3509e`, 2026-09-10); no other query id or finding class is excluded or dismissed by any route. Five doctrine surfaces still describe a per-alert dismissal through the code-scanning route as the owner's act, several in more than one passage, and every such passage is re-trued to the tracked exclusion with the owner's words (a case-insensitive grep for "dismiss" in each file finds them; read at the 2026-09-17 tip): `no-warning-toleration` (the exception sentence under §Rule); `sonarqube-mcp-instructions` (the cardinal anti-pattern paragraph, the one-outcome paragraph, the 2026-07-26 amendment's superseded note and the Security Hotspots paragraph); ADR-219 (§Decision's dismissal paragraph and the consequence bullet "never a repo-wide query exclusion"); `docs/governance/sonar-disposition-policy.md` (§One-Outcome Rule, Disposition Workflow step 1, the fix-first principle bullets and the 2026-09-08 amendment); and `docs/engineering/quality-tooling-mcp-coupling.md` (the TL;DR's CodeQL item, the governing-rule paragraph, §Dismissal, the `js/missing-rate-limiting` row, and §3 and §7 of the quality-driving session). With them, the live clauses of `.agent/plans/delivery/code-scanning-alerts-to-zero.plan.md` (overview, goal, mechanism, criterion 3, out of scope and unit 6), which forbid "a path exclusion or query filter"; its decision-log rows are history and stay. The route comments ADR-219 names stay: they are the architectural claim, and the exclusion is its tracked form.
  `[captured: 2026-09-17 | source: the owner's answer to the consolidation's question on the CodeQL mechanism (read 2026-09-17); the two conflicting records 41235118c and bd7a3509e | target: .agent/rules/no-warning-toleration.md; .agent/rules/sonarqube-mcp-instructions.md; docs/architecture/architectural-decisions/219-rate-limiting-is-an-edge-concern.md; docs/governance/sonar-disposition-policy.md; docs/engineering/quality-tooling-mcp-coupling.md | trigger: the next context of the dedicated consolidation, before the directive pass (none of the five is a directive) | size: the dismissal passages of five surfaces and one plan's live clauses | status: due]`
