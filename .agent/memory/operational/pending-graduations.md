...
- **Tests never, under any circumstances, use or create IO — smoke and E2E included** — owner, 2026-09-14, verbatim: "to be clear, tests never, ever, under any circumstances use or create IO". testing-strategy.md as written admits it in two places — §Philosophy ("E2E and smoke tests may trigger IO only under their respective constraints") and §Test Types ("Smoke tests CAN trigger all IO types, DO have side effects") — and its `.agent/`-substrate rule points at a `mkdtemp` fixture repository; the estate's `smoke-tests/` directory carries real-git and real-filesystem smokes under that text. The ruling closes the door the text leaves open: a smoke proves the shipped artefact starts and answers over its protocol channel; anything a test cannot reach without creating IO is proven by non-test validation kept minimal, or by an observation made once and recorded (the entry below). Worked instance the same night: a smoke that built scratch git repositories to prove a merge predicate, and a unit test creating a temporary directory through a `test-helpers/` module, both removed at the owner's word. Open for the owner at the session, not presumed here: the ruling's reach over the existing estate — the commit-queue smokes create scratch repositories, and two integration tests import `node:fs/promises` under a lint warning (`flag-path-resolve.integration.test.ts`) — whether those become non-test validation kept minimal (the 2026-09-06 wording) or are removed; and whether `no-real-io-in-tests` extends to the helpers tests import, where tonight's dodge passed it.
  `[captured: 2026-09-14 | source: owner ruling 2026-09-14 (napkin, the Landing #143 and #146 block; estate-coordination thread record) | target: .agent/directives/testing-strategy.md §Philosophy IO sentence, §Test Types smoke definition, the mkdtemp sentence under the .agent-substrate rule; PDR-091 named beside it with the 2026-09-06 entry above | trigger: the dedicated consolidation session, under the directive-file-context-budget rule's 30% headroom | size: three sentences and one deletion | status: due]`
- **Sometimes an observation, not an automated check** — owner, 2026-09-14, verbatim: "sometimes you don't need an automated check @validation-strategy.md sometimes you need an observation." validation-strategy §Right tool says a check uses the property's real machinery; the ruling names the case where the real machinery cannot be run by a test (git's own merge semantics; a filesystem; a running vendor): the property is exercised once at cure time by hand, and the run — commands, inputs, what was seen — is recorded on the pull request and in the records as an observation. An observation is dated, first-hand and reproducible from its record; it is never narrated as a suite's proof, and a suite is never built to replace it with IO. Worked instance: the review-cost gate's sync predicate, proven by unit tests over injected git output plus one recorded run of the real git on PR #145's merge (PR #146, 2026-09-14).
