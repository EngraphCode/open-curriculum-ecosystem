...
- **Two peer forks, never an upstream: the cross-fork routing clause encodes a withdrawn relationship model** — owner, 2026-09-16, verbatim: *"there is no 'upstream' there are two forks of OCE. Eventually this fork will be merged back to the Oak fork. All of those issues should be fixed locally, they go to Oak when the fork syncs back, not before."* The `cross-fork-integration` SKILL, under §Authority frame at lines 50 to 52, reads "**Upstream's mechanism is authoritative for shared mechanism.** A reviewer finding about upstream code is never cured on the fork; it is routed as an upstream report or a fork lane, and the thread is resolved on that route." The fork-lane arm is correct. The upstream-report arm, and "never cured on the fork", invert the right default: under peer forks that merge back, a finding on carried code is cure-worthy on this fork, the merge-back is the delivery mechanism, and nothing is ever sent to another project's maintainers. Lines 313 to 317 re-teach the withdrawn model: "On a carrier every review round is findings on someone else's code, so that route is the whole disposition vocabulary the lane needs and the cure-worthy count stays zero unless a finding is about the sync itself (2026-09-09, the 1.179.0 carrier: two rounds, three threads, all routed to one owner-held upstream report, cure-worthy 0)". They change with the clause. The same model is live on three more surfaces, which change with it:
  - the `sync-default-branch-past-a-skip-ci-upstream-tip` runbook, step 4 ("a finding about upstream code is never cured on the sync … an upstream report, or a follow-up lane") and its rollback paragraph;
  - `mcp-487-string-encoded-numbers.plan.md`, lines 66 to 67 ("the finding rides the owner's held upstream-report item");
  - the 1.181.1 draft at `.agent/reports/upstream-sync/upstream-report-draft-1.181.1-sync-2026-09-11.md`, still framed as "one paste-ready note for upstream's maintainers … the owner's to send".

  The cost is measured, not hypothetical: three carriers dispositioned every carried-code finding as routed away, cure-worthy 0 — #99 (1.179.0, three threads), #127 (1.181.1, six findings) and #147 (1.181.3, eight findings). The 1.181.3 set is re-headed as a local work list at `.agent/reports/upstream-sync/upstream-report-draft-1.181.3-sync-2026-09-15.md`; the 1.179.0 and 1.181.1 sets have not been re-examined under the corrected model. Decide at the same time the names that carry the withdrawn model: the `upstream-sync/` directory and the `automation/upstream-carrier-*` branches. Not rewritten at the session close that captured it: canonical doctrine with a live worked instance wants the consolidation session's reading.
  `[captured: 2026-09-16 | source: owner correction 2026-09-16 (napkin 2026-09-16; estate-coordination thread record §2026-09-16 12:32Z; the compaction wrap's verification workflow wf_15bc764e-e59 for the runbook, plan and 1.181.1 surfaces) | target: .agent/skills/change-custody/cross-fork-integration/SKILL-CANONICAL.md — the routing clause at lines 50 to 52 and the worked instance at lines 313 to 317; the sync runbook's step 4 and rollback | trigger: second item of the dedicated consolidation session (after the IO invariant), or before the next sync of carried code, whichever comes first | size: one clause, one worked-instance paragraph and one runbook step rewritten; one plan line and one report re-headed | status: due]`
