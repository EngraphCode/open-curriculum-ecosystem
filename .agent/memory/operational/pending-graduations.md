...
- **Finished history in continuity records and registers is graduated, then archived** — owner decision, 2026-09-17, choosing "Graduate, then archive" over "Curate and delete, add a trigger" and "Decide later; no bulk curation": journal-shaped history is treated like the napkin. Behaviour-changing entries graduate first; the finished history then moves to a dated archive proven byte-identical; only live state stays in place; a named skill step triggers the act; and every surface that grows carries fitness coverage. The frame behind the question: a memory surface's lifecycle has four parts, a role, a disposition for finished content, a trigger (the moment and the skill step), and a proof that nothing was lost, and only the napkin had all four. Evidence read 2026-09-17: `continuity-practice.md` §Disposition of Continuity Surfaces gave `repo-continuity.md` and the thread records a disposition (live stays verbatim; finished work has its insight homed and its residue deleted; "never archive/split/rotate/shard") and a runbook, and no skill or rule invoked the runbook, so the files grew (by the readout, `repo-continuity.md` 806 lines and 73.6 KB, the agentic-engineering-enhancements record 117 KB, the paused mcp-submission-drive record 765 lines, all critical); the estate-coordination thread record (3,659 lines) and the continuity-memory record carried no fitness frontmatter and no `overflow_disposition`, and neither did `frictions-register.md` (4,371 lines, no lifecycle for a cured friction), while `director-handoff.md` had a role and no disposition note; and the runbook's `grep -F` proof missed a curated passage's statement about its neighbour (repo-continuity, 2026-09-17: the only sentence saying an "UNCOMMITTED" block had been committed). What changes: the directive section's disposition and proof (live state in place; finished history graduated, then archived with a byte-identity check, as the napkin rotation does); the `overflow_disposition` note on the 28 memory files that carry it (29 with the directive's own frontmatter); a trigger step in the consolidation skill beside napkin rotation; role notes, dispositions and fitness frontmatter for the records that lack them; and the four-part lifecycle as a candidate concept node.
  `[captured: 2026-09-17 | source: owner decision 2026-09-17 on the dedicated consolidation's lifecycle question; the distilled.md entry of the same date, at 49b6c3ae9 (drained into this entry in 1bebadf58) | target: .agent/directives/continuity-practice.md §Disposition of Continuity Surfaces; the consolidate-docs skill's rotation steps; overflow_disposition and fitness frontmatter on the named memory files | trigger: the directive pass in a fresh context, with careful analysis, before bulk archiving of the large memory files | size: one directive section rewritten, one skill step, frontmatter on about 35 files | status: due]`
