...
Per the check-singleton-per-window invariant: when agent A is running `pnpm check` (or another whole-repo gate sweep) in a working tree other agents share, A emits a broadcast naming the gate, the working tree, the started-at timestamp, and the expected finish window. Those agents observe and do not start a parallel run; seats in separate worktrees run their own gates under `no-unbounded-host-load` item 6.
