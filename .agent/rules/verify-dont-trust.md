...
An instrument's verdict — a front door's refusal, a reviewer's finding, a
tally's count — is a reading of that surface, not the surface: when the seat
can read the state directly, the state governs, and a verdict the state
contradicts is a defect to file against the instrument, never a timer to wait
on (2026-09-12: a front door read a substantive review as a skip marker and the
seat waited on its timeout arm with the settled state in front of it).
