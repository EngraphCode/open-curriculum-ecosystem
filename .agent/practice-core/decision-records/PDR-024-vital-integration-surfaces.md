...
| **Canonical quality-gate naming** | PDR-008 | Stable `clean` / `build` / `format` / `format:fix` / `lint` / `lint:fix` / `typecheck` / `test` / `check` / `fix` / `dev` across ecosystems; per-ecosystem adaptation rule; CI runs every leg of `check` (PDR-008 as amended 2026-09-24) |
